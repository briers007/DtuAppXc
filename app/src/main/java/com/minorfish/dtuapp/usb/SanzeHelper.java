package com.minorfish.dtuapp.usb;
...
public class SanzeHelper {
    private static SanzeHelper sSanzeHelper;
    private Context mContext;

    public SanzeHelper(Context context) {
        mContext = context;
    }

    public static SanzeHelper getInstance() {
        if (sSanzeHelper == null) {
            sSanzeHelper = new SanzeHelper(App.getApp().getApplicationContext());
        }
        return sSanzeHelper;
    }
...
//                mHandler.obtainMessage(DSR_CHANGE).sendToTarget();
    ...
    public void getData() {
        sendMsg(new byte[]{0x01, 0x03, 0x00, 0x10, 0x00, 0x10, 0x45, (byte) 0xC3});
    }
        ...
        public void run() {
            ...
            if (serialPort != null) {
                if (serialPort.open()) {
                    ...
                    Log.e("TTTTTT", "Send broadcast usb ready");
